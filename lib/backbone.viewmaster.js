  ...
  Backbone.ViewMaster = Backbone.View.extend({
    ...
    constructor: function(opts) {
      ...
      **/
      ...
      /**
      * Event current being broadcasted. Used to prevent bubbling.
      * @private
      * @type String
      **/
      this._broadcasting = null;
    },
    ...
    _prepareViews: function(views) {
      ...
      _.each(ensureArray(views), function(view) {
        ...
        // Bind bubbling
        if (!view._bubble) {
          view._bubble = view.bindTo(view, "all", function(event) {
            // Broadcasted events should not bubble up
            if (view._broadcasting === event) return;
    ...
    /**
    * Companion method to event bubbling. It's like `trigger`, but the events
    * are broadcasted all the way down to all children views too.
    *
    * @method broadcast
    * @param {String} event
     * @param {any} [args*] Zero or more objects to be passed along with the
