...
(function() {
  ...
  Backbone.ViewMaster = Backbone.View.extend({
      ...
      ensureArray(views).forEach(function(view) {
        ...
        if (!view._bubble) {
          view._bubble = view.bindTo(view, "all", function(eventName, arg) {
            if (!arg || typeof(arg.parent) === "undefined" || arg.parent) self.trigger(eventName, arg);
          }, view);
        }
    ...
    /**
     * Same as original Backbone trigger, but the event are bubbled up to the
     * parent views unless object `{ parent: false }` or `{ parent: null }` is
     * passed.
     *
     * @method trigger
     * @param {String} event Event name
     * @param {Object} object Event object to pass with the event
     * @return {Object} this
     **/
    // trigger: function() { },
